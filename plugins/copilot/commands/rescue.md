...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model>] [--effort <low|medium|high|xhigh>] [what Copilot should investigate, solve, or continue]"
...
`copilot:copilot-rescue` is a subagent, not a skill — do not call `Skill(copilot:copilot-rescue)` (no such skill) or `Skill(copilot:rescue)` (that re-enters this command and hangs the session). The command runs inline so the `Agent` tool stays in scope; forked general-purpose subagents do not expose it.
...
- If the helper reports that Copilot is missing, stop and tell the user to run `/copilot:setup`.
